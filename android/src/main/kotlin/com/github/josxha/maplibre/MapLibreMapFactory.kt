...
class MapLibreMapFactory(
  ...
  override fun create(context: Context?, id: Int, args: Any?): PlatformView {
    val params = args as? Map<String, Any> ?: throw IllegalArgumentException("Args must be a Map")
    val builder = MapLibreMapBuilder()

    Convert.interpretMapLibreMapOptions(params["options"] as? Map<String, Any>, builder, context)
    if (params.containsKey("initialCameraPosition")) {
      val position = Convert.toCameraPosition(params["initialCameraPosition"] as Map<String, Any>)
...
interface LifecycleProvider {
  fun getLifecycle(): androidx.lifecycle.Lifecycle?
}

// Dummy implementations for Convert and MapLibreMapBuilder classes
object Convert {
  fun interpretMapLibreMapOptions(options: Map<String, Any>?, builder: MapLibreMapBuilder, context: Context?) {
    // Interpret and set options on builder
  }

  fun toCameraPosition(position: Map<String, Any>): CameraPosition {
    // Convert to CameraPosition
    return CameraPosition.Builder().build() // Example placeholder implementation
  }
...
class MapLibreMapBuilder {
  ...
  fun build(id: Int, context: Context?, messenger: BinaryMessenger, lifecycleProvider: LifecycleProvider): PlatformView {
